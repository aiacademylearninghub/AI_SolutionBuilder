...
import {
  getMermaidText,
  updateDiagram,
} from "../pages/SolutionBuilder/mermaidText";
...
export default function CardView({ rootNode, updateCodeSandbox }) {
  ...
  const [architectureDiagram, setArchitectureDiagram] =
    React.useState("graph TD");
  ...
  const handleOnChoiceClick = (choice) => {
    ...
    if (!child && popNodeStackOnInvalidNode(child)) {
      alert(
        "Next question not available for selected choice, switching to next block "
      );
      return;
    }
      ...
      {!current_node && useAI && (
        ...
        >
          <Typography variant="h5">
            Preparing next question for you...
          </Typography>
          <CircularProgress />
        </Box>
      )}
        ...
        >
          ...
          <Box>
            <IconButton
              ...
              sx={{
                display:
                  currentIndexInHistory < stateHistory.length - 1
                    ? "inline-block"
                    : "none",
              }}
            >
      ...
      {current_node && (
        <Box margin="10px">
          ...
          >
            ...
            {getLongestLeafNodeDepth() + currentIndexInHistory + 1}){" "}
